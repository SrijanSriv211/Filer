...
class Filer:
    ...
    def encrypt(self, text: str) -> list:
        ...
        text_to_chunks = self.__split_text_into_chunks__(text, 8)
        chunks_to_num = self.__chucks_to_numbers__(text_to_chunks, ascii_map)

        return [i * self.random_encryption_key**8 for i in chunks_to_num]
    ...
    def decrypt(self, encrypted_chunks: list) -> list:
        ...
        return [format(i / self.random_encryption_key**8, f".0f") for i in encrypted_chunks]
